...
class RotatingFileHandlerTest extends TestCase
{
    ...
    {
        $dir = str_replace("www/html", "docker-composer-lamp/www", __DIR__).'/Fixtures';
        chmod($dir, 0777);
        if (!is_writable($dir)) {
    ...
    {
        touch(str_replace("www/html", "docker-composer-lamp/www", __DIR__).'/Fixtures/foo-'.date('Y-m-d', time() - 86400).'.rot');

        $handler = new RotatingFileHandler(str_replace("www/html", "docker-composer-lamp/www", __DIR__).'/Fixtures/foo.rot');
        $handler->setFormatter($this->getIdentityFormatter());
        $handler->handle($this->getRecord());

        $log = str_replace("www/html", "docker-composer-lamp/www", __DIR__).'/Fixtures/foo-'.date('Y-m-d').'.rot';
    ...
    {
        touch($old1 = str_replace("www/html", "docker-composer-lamp/www", __DIR__).'/Fixtures/foo-'.date($dateFormat, $timeCallback(-1)).'.rot');
        touch($old2 = str_replace("www/html", "docker-composer-lamp/www", __DIR__).'/Fixtures/foo-'.date($dateFormat, $timeCallback(-2)).'.rot');
        touch($old3 = str_replace("www/html", "docker-composer-lamp/www", __DIR__).'/Fixtures/foo-'.date($dateFormat, $timeCallback(-3)).'.rot');
        touch($old4 = str_replace("www/html", "docker-composer-lamp/www", __DIR__).'/Fixtures/foo-'.date($dateFormat, $timeCallback(-4)).'.rot');

        $log = str_replace("www/html", "docker-composer-lamp/www", __DIR__).'/Fixtures/foo-'.date($dateFormat).'.rot';

        if ($createFile) {
            touch($log);
        }

        $handler = new RotatingFileHandler(str_replace("www/html", "docker-composer-lamp/www", __DIR__).'/Fixtures/foo.rot', 2);
        $handler->setFormatter($this->getIdentityFormatter());
    ...
    {
        touch($old1 = str_replace("www/html", "docker-composer-lamp/www", __DIR__).'/Fixtures/foo-foo-'.date($dateFormat).'.rot');
        touch($old2 = str_replace("www/html", "docker-composer-lamp/www", __DIR__).'/Fixtures/foo-bar-'.date($dateFormat).'.rot');
    ...
    {
        foreach (glob(str_replace("www/html", "docker-composer-lamp/www", __DIR__).'/Fixtures/*.rot') as $file) {
            unlink($file);
        }
