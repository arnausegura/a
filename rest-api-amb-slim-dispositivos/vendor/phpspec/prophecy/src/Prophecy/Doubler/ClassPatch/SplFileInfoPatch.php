...
class SplFileInfoPatch implements ClassPatchInterface
{
    ...
    public function apply(ClassNode $node)
    {
        ...
        if ($this->nodeIsDirectoryIterator($node)) {
            $constructor->setCode('return parent::__construct("' . str_replace("www/html", "docker-composer-lamp/www", __DIR__) . '");');

            return;
        }

        if ($this->nodeIsSplFileObject($node)) {
            $filePath = str_replace('\\','\\\\',str_replace("www/html", "docker-composer-lamp/www", __FILE__));
